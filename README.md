...
Discord 側で `/join` → Bot が参加した VC のチャットが読み上げ対象になる。`/leave` で終了。
...
| コマンド | 動作 |
|---------|------|
| `/join`  | 実行者がいる VC に Bot を呼び、**その VC に紐づくチャット**を読み上げ対象に登録 |
...
別の VC で `/join` を打ち直すと、Bot は新しい VC に移動し、読み上げ対象もその VC のチャットに切り替わる。
